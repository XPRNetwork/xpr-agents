...
if (!signingKey && process.env.XPR_ACCOUNT) {
    console.warn('[xpr-agents] A2A_SIGNING_KEY not set — A2A outbound calls disabled. ' +
        'See docs/A2A.md to enable.');
}
function registerA2ATools(api, config) {
    // ── xpr_a2a_discover ──────────────────────────────────────────
    api.registerTool({
    ...
    api.registerTool({
        ...
        handler: async ({ account, job_id, instructions, confirmed }) => {
            ...
            // Confirmation gate
            const confirmation = (0, confirm_1.needsConfirmation)(config.confirmHighRisk, confirmed, 'Delegate Job via A2A', { target_agent: account, job_id, instructions: instructions.slice(0, 100) + '...' }, `Send job #${job_id} context and instructions to agent '${account}' via A2A`);
            if (confirmation)
                return confirmation;
