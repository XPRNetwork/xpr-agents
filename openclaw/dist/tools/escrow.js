...
function registerEscrowTools(api, config) {
    ...
    api.registerTool({
        ...
        handler: async ({ client, agent, state, limit = 20 }) => {
            ...
            const registry = new sdk_1.EscrowRegistry(config.rpc, undefined, contracts.agentescrow);
            const convertList = (result) => ({
                items: result.items.map(j => jobToXpr(j)),
            ...
            if (account) {
                ...
                return { items: filtered.slice(0, limit).map(j => jobToXpr(j)), hasMore: filtered.length > limit };
            }
    ...
    api.registerTool({
        ...
        handler: async ({ job_id, evidence_uri }) => {
            ...
            for (const url of urls) {
                (0, validate_1.validateUrl)(url, 'evidence_uri');
            }
            const registry = new sdk_1.EscrowRegistry(config.rpc, config.session, contracts.agentescrow);
    ...
    api.registerTool({
        ...
        handler: async ({ job_id, confirmed }) => {
            if (!config.session)
                throw new Error('Session required: set XPR_ACCOUNT and ensure proton CLI has the account key in its keychain');
            (0, validate_1.validatePositiveInt)(job_id, 'job_id');
            const confirmation = (0, confirm_1.needsConfirmation)(config.confirmHighRisk, confirmed, 'Claim Job Timeout', { job_id }, `Close out job #${job_id} after its deadline (payment to agent if delivered, refund to client if not)`);
            if (confirmation)
                return confirmation;
            const registry = new sdk_1.EscrowRegistry(config.rpc, config.session, contracts.agentescrow);
    ...
    api.registerTool({
        ...
        handler: async ({ job_id, confirmed }) => {
            if (!config.session)
                throw new Error('Session required: set XPR_ACCOUNT and ensure proton CLI has the account key in its keychain');
            (0, validate_1.validatePositiveInt)(job_id, 'job_id');
            const confirmation = (0, confirm_1.needsConfirmation)(config.confirmHighRisk, confirmed, 'Cancel Job', { job_id }, `Cancel job #${job_id} and refund any escrowed funds to the client`);
            if (confirmation)
                return confirmation;
            const registry = new sdk_1.EscrowRegistry(config.rpc, config.session, contracts.agentescrow);
    ...
    api.registerTool({
        ...
        handler: async ({ job_id, reason, confirmed }) => {
            if (!config.session)
                throw new Error('Session required: set XPR_ACCOUNT and ensure proton CLI has the account key in its keychain');
            (0, validate_1.validatePositiveInt)(job_id, 'job_id');
            (0, validate_1.validateRequired)(reason, 'reason');
            if (reason.length > 512)
                throw new Error('reason must be 1-512 characters');
            const confirmation = (0, confirm_1.needsConfirmation)(config.confirmHighRisk, confirmed, 'Cancel Job As Agent', { job_id, reason }, `Cancel job #${job_id}, forfeiting payment and refunding all escrowed funds to the client`);
            if (confirmation)
                return confirmation;
            const registry = new sdk_1.EscrowRegistry(config.rpc, config.session, contracts.agentescrow);
    ...
    api.registerTool({
        ...
        handler: async ({ job_id }) => {
            ...
            return { bids: bids.map(b => bidToXpr(b)), count: bids.length };
        },
    });
    api.registerTool({
        name: 'xpr_submit_bid',
        ...
        handler: async ({ job_id, amount, timeline, proposal, confirmed }) => {
            ...
            if (amount <= 0)
                throw new Error('amount must be positive');
            (0, validate_1.validateAmount)((0, validate_1.xprToSmallestUnits)(amount), config.maxTransferAmount);
            (0, validate_1.validatePositiveInt)(timeline, 'timeline');
            (0, validate_1.validateRequired)(proposal, 'proposal');
            const confirmation = (0, confirm_1.needsConfirmation)(config.confirmHighRisk, confirmed, 'Submit Bid', {
                job_id,
                amount: `${amount} XPR`,
            ...
            return registry.submitBid({
                job_id,
                amount: (0, validate_1.xprToSmallestUnits)(amount),
                timeline,
                proposal,
            });
        },
