...
${BOLD}SIGNING KEY:${NC}
    This script does NOT take a private key. Signing is performed by
...
${BOLD}EXAMPLES:${NC}
    ...
    ./setup.sh --account myagent --api-key sk-ant-xxx

    # Mainnet deployment
    ./setup.sh --account myagent --api-key sk-ant-xxx --network mainnet
...
if ! command -v proton &>/dev/null; then
  ...
  warn "proton CLI keychain is empty. Add your key before starting the agent:"
  echo "    proton key:add"
else
  success "proton CLI keychain populated"
fi
