...
// spoof X-Forwarded-For and dodge or poison the per-IP limit. Default 1 (this
// service runs behind Railway's proxy); a directly-exposed deployment should set
// TRUST_PROXY=0. An unparseable value falls back to 1, not NaN.
app.set('trust proxy', Number.isInteger(Number(process.env.TRUST_PROXY)) ? Number(process.env.TRUST_PROXY) : 1);
