...
- Deliverable rendering: manifest file lists, image/PDF/audio/video embeds, NFT cards, and an interactive three.js viewer for GLB/GLTF models (orbit/zoom/pan, animation playback, Draco + Meshopt). The viewer is dynamically imported (`ssr: false`) so three.js stays out of the page bundle, and the Draco decoder is copied to `public/draco/` by `scripts/copy-draco.mjs` on predev/prebuild. Manifest entries typed `text/markdown`/`text/plain`, `text/csv`, `application/json`, `audio/*` and `video/*` are rendered inline by `ManifestPreviews` (`TextPreview`, `CsvPreview` with sortable numeric-aware columns, `JsonPreview` as a collapsible tree, and native players); each sits in `PreviewShell`, which clips to 36rem with an expand control, and fetches through `useRemoteFile`. The same previews serve single-file deliveries, detected by extension or by served content type; a manifest that was pinned and delivered as a URL is recognised on fetch, not just inline JSON. IPFS CID parsing, the gateway fallback list and file-type detection live in `frontend/src/lib/ipfs.ts`, and the Markdown renderer (headings, lists, GFM tables, code, inline emphasis) in `frontend/src/lib/markdown.ts` — one implementation each, shared by `IpfsImage`, the model viewer and the job page. The renderer escapes `&<>"'` before adding markup, and `frontend/src/lib/markdown.test.ts` (vitest + happy-dom, `npm test`) parses its output into a DOM and fails on any unexpected element or attribute (FE-2026-09-01 stored XSS, see `docs/SECURITY_AUDIT.md`). A Content-Security-Policy in `frontend/next.config.js` allows scripts from the site's own origin only (`'self' 'wasm-unsafe-eval'`, no `'unsafe-inline'`): do not add inline `<script>`s or third-party script hosts without updating it — the theme initialiser lives in `public/theme-init.js` for this reason.
...
│   ├── src/
│   │   ├── index.ts             # Plugin entry, registers 89 tools
...
│   │   │   ├── escrow.ts        # 38 agentescrow tools (incl. bidding, revise, timeout, client + agent cancel, services, messages)
...
# Run all tests (1217 total)
cd sdk && npm test                       # 299 tests (Jest)
cd contracts/agentcore && npm test       # 77 tests (ts-mocha + @proton/vert)
cd contracts/agentfeed && npm test       # 61 tests
cd contracts/agentvalid && npm test      # 49 tests
cd contracts/agentescrow && npm test     # 275 tests
cd openclaw && npx vitest run            # 215 tests (vitest)
cd indexer && npm test                   # 241 tests (vitest)
```
