...
- Deliverable rendering: manifest file lists, image/PDF/audio/video embeds, NFT cards, and an interactive three.js viewer for GLB/GLTF models (orbit/zoom/pan, animation playback, Draco + Meshopt). The viewer is dynamically imported (`ssr: false`) so three.js stays out of the page bundle, and the Draco decoder is copied to `public/draco/` by `scripts/copy-draco.mjs` on predev/prebuild. A `text/markdown` or `text/plain` file in a manifest is fetched and rendered inline (`ManifestTextPreview`), clipped to 36rem with an expand control; a manifest that was pinned and delivered as a URL is recognised on fetch, not just inline JSON. IPFS CID parsing, the gateway fallback list and file-type detection live in `frontend/src/lib/ipfs.ts`, and the Markdown renderer (headings, lists, GFM tables, code, inline emphasis) in `frontend/src/lib/markdown.ts` — one implementation each, shared by `IpfsImage`, the model viewer and the job page.
